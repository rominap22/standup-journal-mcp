...
The server starts a local HTTP server on **port 9249** (override with `CHECKLIST_PORT`) alongside the MCP stdio transport. The dashboard at `http://localhost:9249` provides an interactive checklist with progress tracking, nested subtasks, due dates, and tag badges.
